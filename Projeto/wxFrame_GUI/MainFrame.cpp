...
void MainFrame::btnClickCoordenograma(wxCommandEvent& event)
{
    ...
    for(auto item : m_Lista){
        if(item.m_Tipo==1){ //Rele 50/51
            ...
            CorrenteAux = CorrenteInicioPlot;
            for(int i = CorrenteInicioPlot; i < CorrenteFimPlot; ++i) {
                if(CorrenteAux < item.m_Corrente){pontoANSI.emplace_back(-10);} //Assim não aparece no plot
                else if(round(CorrenteAux) == round(item.m_Corrente)){pontoANSI.emplace_back(log10(item.m_Tempo));}
                else{pontoANSI.emplace_back(1000);}                              //Assim não aparece no plot
                CorrenteAux += 1;
            }
            ...
            CorrenteAux = CorrenteInicioPlot;
            for(int i = CorrenteInicioPlot; i < CorrenteFimPlot; ++i) {
                if(CorrenteAux < item.m_Corrente){pontoINRUSH.emplace_back(-10);} //Assim não aparece no plot
                else if(round(CorrenteAux) == round(item.m_Corrente)){pontoINRUSH.emplace_back(log10(item.m_Tempo));}
                else{pontoINRUSH.emplace_back(1000);}                              //Assim não aparece no plot
                CorrenteAux += 1;
            }
    ...
    //Alterar para os valores na ordem: Mínima corrente (x), Máxima corrente (x), MínimoMáximo Tempo (y), Máximo tempo (y)
